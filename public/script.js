...
for (var i = 0, ll = links.length; i < ll; i++) {
	var el = links[i];
	if (
		el.className.indexOf("next") + el.className.indexOf("prev") == -2 ||
	...
	if (data.source) {
		...
		if (data.license) {
			$(monot).append(
				' [<a href="' +
					data.license +
					'"><img src="http://image.frood.io/cc.png" class="cc" style="width: 15px" /></a>]'
			);
		}
		...
		mu: function () {
			var bgs = {
				...
				images: function () {
					...
					return (
						"#AAA url('http://image.frood.io/logobg.jpg') no-repeat " +
						-200 * Math.floor(Math.random() * 5) +
						"px " +
						-200 * Math.floor(Math.random() * 3) +
						"px"
					);
				},
			};
			var type = pokes == 3 ? "images" : "colors";
			_$.css({ background: bgs[type]() });
			return this;
		},
		poke: function () {
			pokes++;
...
Date.prototype.ddate = function () {
	...
	var fin = dom % 10;
	return (
		(holy.hasOwnProperty(doy) ? holy[doy] + ", " : "") +
		planets[this.getDay()] +
		"<br />" +
		days[doy % 5] +
		" " +
		...
		" " +
		dom +
		(fin > 0 && dom != 11 && dom != 12 && dom != 13 && fin <= 3
			? suffix[fin - 1]
			: "th") +
		", " +
		("00000" + (this.getFullYear() + 1166)).substr(-5) +
		" YOLD"
	);
};
	...
	$(this)
		...
		.mouseout(function (event) {
			...
			if ($this.data("remorph")) {
				...
				$this.data("remorph", "");
				setTimeout(function () {
					$this.attr("src", rem).data("morph", src).removeClass("unob");
				}, 1000);
			}
		});
});
