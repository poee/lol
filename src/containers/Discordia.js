...
class Discordia extends Component {
	...
	render() {
		...
		const slug = ((history.location && history.location.hash) || "").match(
			/#((\d+)|(\w+))$/
		);
		const page = slug
			? slug[2]
				? zeroPad(slug[2])
				: slug[1] // For numeric slugs, zero pad
			: "front"; // Default page
		...
		const metaIdx = PAGES.findIndex((meta) =>
			typeof meta === "string" ? meta === page : meta.slug === page
		);
		if (metaIdx !== -1) {
		...
		return (
			<div className="discordia">
				<DocumentTitle title={`${title}${TITLE}`}>{content}</DocumentTitle>
				<hr />
				<nav>
					{prevPage && (
